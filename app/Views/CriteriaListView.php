...
<div class="p-8 bg-gray-50 min-h-screen">
			...
			<table class="w-full">
				...
				<thead class="bg-gray-50 border-b border-gray-200">
					<tr>
						...
						<th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">CRITERIA NAME</th>
						<th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">TYPE</th>
						<th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">WEIGHT</th>
					...
					<tbody class="divide-y divide-gray-200">
						<?php foreach ($criteriaList as $criteria): ?>
							<tr class="hover:bg-gray-50 transition">
								...
								<!-- Criteria Name -->
								<td class="px-6 py-4">
									<p class="font-semibold text-gray-900"><?= htmlspecialchars($criteria['criteriaName']) ?></p>
								</td>
								...
								<td class="px-6 py-4 w-40">
									<div class="flex items-center gap-2">
										<button class="delete-btn px-3 py-1.5 bg-red-600 text-white text-xs font-semibold rounded-lg transition hover:bg-red-700 hover:shadow-lg" 
											type="button"
				...
				<?php else: ?>
					<tbody>
						<tr>
							<td colspan="4" class="px-6 py-8 text-center text-gray-500">No criteria found</td>
						</tr>
					</tbody>
				<?php endif; ?>
			</table>
    ...
    <!-- Subcriteria Table -->
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden mt-9">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 class="text-lg font-bold text-gray-900">Sub Criteria List</h3>
            <p class="text-sm text-gray-600 mt-1">View all subcriteria and their variable values</p>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <!-- Table Header -->
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">SUB ID</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">CRITERIA</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">SUB NAME</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">VARIABLE VALUE</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">COMMENT</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">ACTIONS</th>
                    </tr>
                </thead>
                <?php if (!empty($subcriteriaList)): ?>
                    <tbody class="divide-y divide-gray-200">
                        <?php foreach ($subcriteriaList as $subcriteria): ?>
                            <tr class="hover:bg-gray-50 transition">
                                <!-- Sub ID -->
                                <td class="px-6 py-4 w-16">
                                    <span class="inline-flex items-center justify-center px-2 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded">
                                        <?= htmlspecialchars($subcriteria['idSub']) ?>
                                    </span>
                                </td>

                                <!-- Criteria -->
                                <td class="px-6 py-4 w-40">
                                    <p class="font-semibold text-gray-900"><?= htmlspecialchars($subcriteria['criteriaName'] ?? 'N/A') ?></p>
                                </td>   

                                <!-- Sub Name -->
                                <td class="px-6 py-4">
                                    <p class="text-gray-700"><?= htmlspecialchars($subcriteria['subName']) ?></p>
                                </td>

                                <!-- Variable Value -->
                                <td class="px-6 py-4 w-40">
                                    <p class="text-sm text-gray-600 font-mono"><?= htmlspecialchars($subcriteria['variableValue']) ?></p>
                                </td>

                                <!-- Comment -->
                                <td class="px-6 py-4">
                                    <p class="text-sm text-gray-600"><?= htmlspecialchars(substr($subcriteria['comment'] ?? '-', 0, 50)) ?><?= strlen($subcriteria['comment'] ?? '') > 50 ? '...' : '' ?></p>
                                </td>

                                <!-- Actions -->
                                <td class="px-6 py-4 w-40">
                                    <div class="flex items-center gap-2">
                                        <button class="delete-sub-btn px-3 py-1.5 bg-red-600 text-white text-xs font-semibold rounded-lg transition hover:bg-red-700 hover:shadow-lg" 
                                            type="button"
                                            title="Delete subcriteria"
                                            data-id="<?= htmlspecialchars($subcriteria['idSub']) ?>">
                                            <i class="bi bi-trash"></i> Delete
                                        </button>
                                    </div>
                                </td>
                ...
                <?php else: ?>
                    <tbody>
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-gray-500">No subcriteria found</td>
                        </tr>
                    </tbody>
                <?php endif; ?>
            </table>
